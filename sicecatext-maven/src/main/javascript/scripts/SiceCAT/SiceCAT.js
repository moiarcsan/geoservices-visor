    ...
    .extend(
        Ext.Component, {
                            ...
                            .loadSearchServices(function() {
                                this_
                                    .loadMapConfiguration(function() {
                                        this_
                                            .overrideConfiguration(this_.jsonMapConfiguration[
                                                "openLayersProxyHost"]);
                                        this_.initGPCL();
                        ...
                        var panel = new SiceCAT.ZoomToResultPanel({
                            ...
                            listeners: {
                                ...
                                loadexception: function() {
                                    ...
                                    if (!!this.idMunicipio) {
                                        msg += "SIGESCAT esta fallando o el id de municipio " + this.idMunicipio +
                                            " es incorrecto!!";
                                    } else if (!!this.idComarca) {
                                        msg += "SIGESCAT esta fallando o el id de comarca " + this.idComarca +
                                            " es incorrecto!!";
                                    } else {
                ...
                if (!!this.jsonLayers) {
                    for (var i = 0; i < this.jsonLayers.length; i++) {
                        ...
                        var url = (OpenLayers.ProxyHost
                            .indexOf("url2") != -1 ? OpenLayers.ProxyHost : "") + layerToLoad['url'];
                        //console.log("URL --> "+ url);
            ...
            testLayer: function(url, map, layerToLoad, continueLoading) {
                ...
                Ext.Ajax.request({
                    ...
                    success: function(result, request) {
                        ...
                        if (!!result && ((!!result.responseText && result.responseText.indexOf(
                                "HTTP 404") < 0) || (!!result.responseXML && result.responseXML.indexOf(
                                "HTTP 404") < 0))) {
                            //this_.loadCorrectLayer(map, layerToLoad);
                        } else {
                        ...
                        if (this_.waiting == 0) {
                            if (this_.errors_text != "") {
                                this_.showErrorWindow(String.format(this_.errorLayersTitleText, this_.errors
                                        .length),
                                    this_.errorLayersBodyInfoText, this_.errors_text);
                            }
                ...
                Ext.Ajax.request({
                    ...
                    success: function(result, request) {
                        ...
                        if (!!result && ((!!result.responseText && result.responseText.indexOf(
                            ...
                            if (!!onError) {
                                this_instance.showErrorWindow(String.format(this_instance.errorLayersTitleText,
                                        this_instance.errors.length),
                                    this_instance.errorLayersBodyInfoText, this_instance.errors_text
                                );
                                onError();
                            } else {
            ...
            loadCorrectLayer: function(map, layerToLoad) {
                ...
                if (layerToLoad['type'] == 'WMS') {
                    var layer_url = Sicecat.getURLProxy(Sicecat.confType, Sicecat.typeCall.CARTOGRAFIA, layerToLoad[
                        'url']);
                    ...
                    if (!!subgroup) {
                        wms.groupLayers = group + "-" + subgroup;
                    }
            ...
            removeLayer: function(layer) {
                if (!!layer) {
                    if (!!layer.layerID && !layer.isBaseLayer) {
                        this.LAYERS_LOADED[layer.layerID] = null;
                    }

                    try {
                        if (!!layer && !!layer.name && map.getLayersByName(layer.name).length > 0)
            ...
            refreshLayers: function(continueLoading) {
                ...
                var parser = new SiceCATGeoParser({
                    map: this.map
                });
                    ...
                    for (var i = 0; i < treeFolder.keys.length; i++) {
                        ...
                        if (!!label) {
                            ...
                            if (label != 'overlays') {
                                ...
                                if ((!!treeFolder.item(key) && treeFolder.item(key).length > 0)) {
                                    ...
                                    this.getTreeRecursive(treeFolder.item(key), tree, notGroupLayers, folder,
                                        idGroup, idUser, parser);
                                } else {
            ...
            loadLayersByGroup: function(idGroup, continueLoading) {
                ...
                parser.loadLayersByGroup(idGroup, function(layers, layerTree, rootFolder) {
                    ...
                    for (var i = 0; i < parser.treeFolder.keys.length; i++) {
                        parser.treeFolder.add(parser.treeFolder.keys[i], parser.treeFolder.get(parser.treeFolder
                            .keys[i]));
                    }
                    this_.getTreeRecursive(parser.treeFolder, this_.auxGroupLayers, this_.notGroupLayers,
                        null, idGroup, null, parser);
                    ...
                    if (!!continueLoading && this_.countLoadingLayers == 0) {
                        continueLoading();
                    }
                });
            },
            ...
            loadMapConfiguration: function(continueLoading, loadMap) {
                ...
                if (!!this.jsonMapConfiguration) {
                    ...
                    parser.loadMapConfiguration(function(form, action) {
                        // ON SUCCESS
                        this_instance.jsonMapConfiguration = Ext.util.JSON.decode(action.response.responseText)
                            .data;
                        if (!!loadMap)
                            this_instance.loadMapConfigurationPost(map);
                        continueLoading();
            ...
            initMapSicecat: function(maxExtent, initialExtent) {
                ...
                console
                    .log("Init map in extent (" + initialExtent + ") on projection '" + this.jsonMapConfiguration[
                        'projection'] + "'");
                ...
                if (!!this.jsonStyles) {
                    ...
                    for (var i = 0; i < this.jsonSearchServices.length; i++) {
                        if (this.jsonSearchServices[i]['type'] == "wfs") {
                            ...
                            if (!!layerProperties && !!layerProperties["url"]) {
                                layerProperties["url"] = OpenLayers.ProxyHost + layerProperties["url"];
                            }
                            ...
                            if (integrator.sicecatInstance.permisos && integrator.sicecatInstance.permisos.editWFS) {
                            ...
                            if (group != 'editables') {
                                ...
                                if (!!subgroup) {
                                    wfs.groupLayers = group + "-" + subgroup;
                                }
                            ...
                            if (this.map && this.map.getLayersByName(wfs.name).length == 0) {
                    ...
                    .each(
                        ...
                        function(c, i) {
                            ...
                            if (c.id
                                ...
                                c.setLayer(layer);
                            } else if (c.name && c.name == "OpenLayers.Control.DeleteFeature") {
                                c.setLayer([layer]);
                            } else if (c.name && (c.name == "DrawPolygon" || c.name == "DrawLine" || c.name ==
                                    "DrawPoint")) {
                                c.layer = layer;
                            }
                        });
            ...
            addSelected: function(feature) {
                ...
                var id;
                if (!!feature.data && !!feature.data.pk_id && !isNaN(feature.data.pk_id)) {
            ...
            clearSelected: function(feature) {
                ...
                if (!!feature.style && !!Sicecat.activeLayer) {
                    Sicecat.activeLayer.drawFeature(feature,
                        feature.style);
                } else if (!!Sicecat.activeLayer && !!Sicecat.activeLayer.styleMap.styles["default"].defaultStyle) {
                    Sicecat.activeLayer
                        .drawFeature(
                            feature,
            ...
            doDrag: function(feature, pixel) {
                ...
                feature.geometry.move(res * (pixel.x - Sicecat.lastPixel.x), res * (Sicecat.lastPixel.y - pixel.y));
            ...
            isOwnerlayer: function(layer) {
                return (Sicecat.GROUP_IDS.SUPERADMIN == layer.groupID && (Sicecat.user.permisos.indexOf("admin1") >
                    -1)) || (Sicecat.GROUP_IDS.CECAT == layer.groupID && (Sicecat.user.permisos.indexOf(
                    "admin2") > -1)) || ((Sicecat.GROUP_IDS.GPCL == layer.groupID) && (Sicecat.user.permisos.indexOf(
                    "admin2") > -1)) || (!layer.groupID
            ...
            isSigescatOk: function() {
                ...
                if (Sicecat.defaultWMSServer != null && Sicecat.defaultWMSServer.indexOf("wms") != -1) {
                    ...
                    Ext.Ajax.request({
                        url: Sicecat.getURLProxy(Sicecat.confType, Sicecat.typeCall.ALFANUMERICA, wfs_url) +
                            "request=" + request_getCapabilities + "&service=" + service + "&version=" +
                            version,
                        success: function(response) {
                            // Send GetFeature from first entity
                            if (response.responseXML != null && response.responseXML.getElementsByTagName(
                                    "FeatureType") != null && response.responseXML.getElementsByTagName(
                                    "FeatureType")[1] != null) {
                                var feature_type = response.responseXML.getElementsByTagName(
                                    "FeatureType")[1];
                                name = feature_type.getElementsByTagName("Name")[0].textContent;
                            }
                            Ext.Ajax.request({
                                url: Sicecat.getURLProxy(Sicecat.confType, Sicecat.typeCall.ALFANUMERICA,
                                        wfs_url) + "request=" + request_getFeature +
                                    "&version=" + version + "&typeName=" + name +
            ...
            doOpenLSRequest: function(posX, posY, funct) {
                var data =
                    '<XLS xsi:schemaLocation="http://www.opengis.net/xls" version="1.2.0" xmlns="http://www.opengis.net/xls"' +
                    ' xmlns:gml="http://www.opengis.net/gml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">' +
                    '<Request methodName="Geocode" requestID="123" ' + 'version="1.2.0" maximumResponses="1">' +
                    '<ReverseGeocodeRequest> <Position> <gml:Point srsName="' + map.getProjection() + '">' +
                    '<gml:pos>' + posX + ' ' + posY + '</gml:pos></gml:Point></Position></ReverseGeocodeRequest>' +
                    '</Request></XLS>';
