...
SiceCAT.widgets.SigescatCercaSolrGeneralSearcherCombo = Ext.extend(Ext.form.ComboBox, {
    /** api: config[map]
    ...
    onLoad: function(){
        ...
        for(var i=0; i<dataSource.records.length; i++){
        	dataSource.records[i].data.type = "cercasolr";
        }
    ...
    getFilterIdProperty: function(featureType){
		...
		if(featureType == "s:k"){
			idProperty = "ID";
		}else if(featureType == "s:i"){
			idProperty = "INE_NUM";
		}else if(featureType.indexOf("p:")>=0){
			idProperty = "ID";
		}
...
Ext.reg('gx_sicecat_sigescatcercasolrgeneralsearchercombo', SiceCAT.widgets.SigescatCercaSolrGeneralSearcherCombo);
